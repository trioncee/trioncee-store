...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen w-full bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white via-neutral-100 to-neutral-200 overflow-hidden flex flex-col items-center">
      ...
      <div className="relative z-20 w-full max-w-[1600px] flex flex-col items-center h-full mx-auto min-h-screen">
        ...
        >
          <h3 className="text-[12px] font-bold tracking-[0.4em] uppercase text-neutral-500 mb-3">Premium T-Shirts</h3>
          <p className="text-[10px] tracking-[0.4em] uppercase text-neutral-400 mb-8">Built for Everyday Style</p>
          <Link to="/shop" className="group relative inline-flex items-center gap-4 px-10 py-3 bg-transparent border border-neutral-400 text-[10px] font-bold uppercase tracking-[0.25em] hover:bg-neutral-800 hover:text-white hover:border-neutral-800 transition-all duration-300 rounded-lg shadow-none active:scale-95 text-neutral-600">
            Shop T-Shirts
          </Link>
        </motion.div>
      </div>
    ...
    <div className="max-w-[1400px] mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
        <div className="max-w-xl">
          <span className="text-[10px] font-bold uppercase tracking-[0.7em] text-neutral-400 mb-4 block">New Arrivals</span>
        </div>
        <Link to="/shop" className="group flex items-center gap-4 text-[11px] font-bold uppercase tracking-[0.5em] hover:opacity-70 transition-all border-b border-neutral-200 pb-2">
        ...
        {products.slice(0, 3).map((product, idx) => (
          <motion.div key={product.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1, duration: 1 }}>
            <Link to={`/product/${product.id}`} className="group block">
              <div className="aspect-[4/5] bg-neutral-100 overflow-hidden mb-6 relative rounded-sm shadow-sm border border-neutral-200 flex items-center justify-center">
                {product.images[0] ? (
              ...
              <div className="space-y-3 text-center md:text-left">
                <h3 className="text-[12px] font-bold tracking-[0.3em] uppercase text-neutral-800">{product.name}</h3>
                <div className="flex items-center justify-center md:justify-start gap-5 text-[10px] text-neutral-400 uppercase tracking-[0.4em] font-medium">
                  <span>{product.category}</span><span className="w-1 h-1 bg-neutral-200 rounded-full"></span><span className="text-black font-bold">₹{product.price}</span>
                </div>
              </div>
            </Link>
          </motion.div>
...
const TeamSection = () => (
  // ... (keep team section)
  <section className="py-10 md:py-14 px-6 md:px-12 bg-gradient-to-b from-[#1B4079] to-[#112d5a] text-white">
    <div className="max-w-[1400px] mx-auto">
...
const Home = ({ products }: { products: Product[] }) => (
  <motion.div
    ...
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
    className="w-full hero-gradient min-h-screen bg-neutral-50"
  >
    <HeroSection />
